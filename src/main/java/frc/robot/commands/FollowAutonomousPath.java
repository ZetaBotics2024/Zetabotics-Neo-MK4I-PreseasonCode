...
public static PathPlannerPath reflectPath(PathPlannerPath original) {
  ...
  points.forEach((PathPoint point) -> {
    point = new PathPoint(new Translation2d(Constants.FieldConstants.kWidth - point.position.getX(), Constants.FieldConstants.kLength - point.position.getY()), point.holonomicRotation);
    newPoints.add(point);
  });
...
public static PathPlannerPath rotatePath(PathPlannerPath original) {
  ...
  List<PathPoint> newPoints = new ArrayList<PathPoint>();
  points.forEach((PathPoint point) -> {
    point = new PathPoint(new Translation2d(Constants.FieldConstants.kWidth - point.position.getX(), point.position.getY()), point.holonomicRotation);
    newPoints.add(point);
  });
  PathPlannerPath newPath = PathPlannerPath.fromPathPoints(newPoints, original.getGlobalConstraints(), original.getGoalEndState());
  return newPath;
